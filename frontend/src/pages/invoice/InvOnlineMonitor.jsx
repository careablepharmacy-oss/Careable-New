...
import { CreditCard, CheckCircle2, XCircle, Clock, AlertTriangle, Search, ArrowLeft } from 'lucide-react';
...
export default function InvOnlineMonitor() {
  ...
  const fetchData = useCallback(async () => {
    try { const res = await monitorAPI.online(); setData(res.data); } catch { toast.error('Failed to load'); }
    setLoading(false);
  }, []);
  ...
  const filtered = invoices.filter(inv => {
    ...
    const ts = inv.tracking_status || 'payment_received';
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate('/invoice-manager')} data-testid="online-back"><ArrowLeft className="w-4 h-4" /></Button>
        <div>
          <h1 className="text-xl font-bold">Online Payment Monitor</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {TRACKING_OPTIONS.map(opt => (
          <button key={opt.value} onClick={() => setFilterStatus(filterStatus === opt.value ? 'all' : opt.value)}
            className={`p-2 rounded-lg text-center transition ${filterStatus === opt.value ? 'ring-2 ring-emerald-500' : ''} ${opt.color}`} data-testid={`online-filter-${opt.value}`}>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input value={search} onChange={e => setSearch(e.target.value)} className="pl-10" placeholder="Search..." data-testid="online-search" /></div>
        <Select value={filterStatus} onValueChange={setFilterStatus}><SelectTrigger className="w-40" data-testid="online-status-filter"><SelectValue placeholder="All" /></SelectTrigger><SelectContent><SelectItem value="all">All Status</SelectItem>{TRACKING_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent></Select>
      </div>
      ...
      <Card><CardContent className="p-0"><Table>
        ...
        <TableBody>
          {filtered.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-6 text-gray-400">No online orders</TableCell></TableRow> :
            filtered.map(inv => (
              <TableRow key={inv.invoice_id} data-testid={`online-row-${inv.invoice_id}`}>
